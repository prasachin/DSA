...
void solve()
{
    ...
    vector<int> slots(maxDeadline + 1, -1); // slots[i] will be the job index that is assigned to the slot i
    ...
    for (const auto &job : v)
    {
        for (int j = job.first; j > 0; j--)
        {
            if (slots[j] == -1)
            {
                slots[j] = job.second; // Assign job to this slot
                res += job.second;     // Add the profit
